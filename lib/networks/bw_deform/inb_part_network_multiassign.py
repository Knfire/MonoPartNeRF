...
class TPoseHuman(GradModule):
    ...
    def get_svd_feat(self, xyz_sampled, idx, k):
        ...
        for idx_axis in range(len(self.coord_line)):  # for x/y/z directions
            axis_feat = torch.sum(coord[idx_axis] * feat[idx_axis], dim=1)  # sum over n_component dimension
            pose_features.append(axis_feat)  # shape: [k, H*W]
