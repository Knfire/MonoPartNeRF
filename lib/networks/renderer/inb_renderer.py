...
class Renderer:
    ...
    def get_pixel_value(self, ray_o, ray_d, near, far, occ, batch):
        ...
        if 'loss_consis' in ret:
            loss_consis = ret['loss_consis'].view(n_batch, -1)
            ret.update({'loss_consis': loss_consis})

        # if 'smooth_loss' in ret:
        #     smooth_loss = ret['smooth_loss'].view(n_batch, -1)
        #     ret.update({'smooth_loss': smooth_loss})
