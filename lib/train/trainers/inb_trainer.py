...
class NetworkWrapper(nn.Module):
    ...
    def forward(self, batch, epoch=-1, split='train'):
        ...
        if split == 'val':
            ...
            if 'loss_consis' in ret:
                loss_consis = ret['loss_consis'].mean()
                scalar_stats.update({"loss_consis": loss_consis})
                loss += cfg.loss_consis_weight * loss_consis

            # if 'smooth_loss' in ret:
            #     smooth_loss = ret['smooth_loss'].mean()
            #     scalar_stats.update({"smooth_loss": smooth_loss})
            #     loss += cfg.smooth_loss_weight * smooth_loss
