...
class Dataset(data.Dataset):
    def __init__(self, data_root, human, ann_file, split, index_a, index_b):
        ...
        all_params = np.load(os.path.join('data/params/{}.npy'.format(cfg.train_dataset.human)),
                             allow_pickle=True).item()['pose']
        self.trained_params = all_params[
                              i:i + ni * i_intv][::i_intv]

        # frame_name_a = f'{self.index_a:06d}'
        # frame_name_b = f'{self.index_b:06d}'
        # bgcolor = np.zeros(3)
        #
    ...
    def prepare_input(self, i):
        ...
        return wxyz, pxyz, A, big_A, pbw, Rh, Th, poses
    ...
    def __getitem__(self, index):
        ...
        E = np.eye(4)  # (4, 4)
        E[:3, :3] = np.array(self.cams['R'][cam_ind]).astype('float32')
        Ts = np.array(self.cams['T'])[cam_ind].astype('float32') / 1000.
        E[:3, 3] = Ts[:3, 0]
        E = E.astype('float32')
        ...
        fuv = np.load(os.path.join(self.data_root, 'uv_coordinates', f'{i}.npy'))
        head_uv = None
        try:
        ...
        wpts, ppts, A, big_A, pbw, Rh, Th, poses = self.prepare_input(i)
